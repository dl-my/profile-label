...
func GetBaseLabel(url string, cookies []*network.Cookie) error {
...
func GetBasePage(url string, cookies []*network.Cookie) (int, error) {
	tag := make(map[string]string)
	note := make(map[string]string)
	//url := "https://basescan.org/mynotes_address"
	...
	// 查找所有含有 data-highlight-target 的 span 标签
	text := strings.Split(doc.Find("li.page-item.disabled > span.page-link.text-nowrap").Text(), " ")
	page, err := strconv.Atoi(text[len(text)-1])
	if err != nil {
		return 0, err
	}
	return page, nil
...
func GetBaseCookie(loginURL, username, password string) ([]*network.Cookie, error) {
	// 连接 Chrome
